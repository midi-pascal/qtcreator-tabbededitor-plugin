...
#define DEBUG qDebug().nospace() << "[" << this->metaObject()->className() << "::" << __FUNCTION__ << "]"
    ...
    m_tabWidget(new QTabWidget(this))
{
	...
	// Get a hook when the IDE is ready to call restoreSession()
	Core::ICore *iCore = Core::ICore::instance();
	connect(iCore, SIGNAL(coreOpened()), this, SLOT(restoreSession()));
}
...
void TabsForEditorsWidget::restoreSession()
{
	DEBUG << " Begin";
	QList<Core::DocumentModel::Entry *> entries = Core::DocumentModel::entries();
	foreach (Core::DocumentModel::Entry* entry, entries) {
		DEBUG << " Opening Entry Name: " << entry->displayName();
		bool Flag = true;
		Core::IEditor* ed = Core::EditorManager::openEditor(entry->fileName(), Core::Id(), Core::EditorManager::OpenEditorFlag::NoFlags, &Flag);
		DEBUG << " Created editor for file: " << ed->document()->filePath();
	}
	DEBUG << " End";
}
